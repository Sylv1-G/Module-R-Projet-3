...
À partir d'une géométrie de base, il est possible de trouver les partitions des documents d'urbanismes en passant par les codes INSEE de toutes les communes présentes dans la zone.
...
amiens <- get_apicarto_gpu("05183", ressource = "municipality")
...
Un filtre a déja été effectué selon les enjeux patrimoniaux, cependant tout ces enjeux ne sont pas forcément shouaités par l'utilisateur. Une fonction permet de filtrer à la main le type de données à extraire. Un exemple de saisie ci-dessous :  
...
La fonction *affichage* permet d'afficher une carte intéractives des prescirptions, des informations ou des SUP. La légende et l'affichage commence par
...
L'exemple ci-dessous affiche les prescriptions.
...
- *dico* qui contient codes permettant de filtrer l'ensemble des prescriptions, informations et SUP en lien avec les enjeux patrimoniaux. 
...
La sortie de la fonction contient une liste des quatres tableau de données (prescriptions, informations, générateurs de SUP et assiettes des SUP).

# Discussion-Conclusion

## Possibilité d’amélioration complémentaire ayant trait aux améliorations législatives

En outre, il serait envisageable d’inclure aux résultats une synthèse des articles de loi et règlements liés aux prescriptions, informations et SUP présentes sur la zone d’étude. Cela permettrait d’afficher directement les conséquences de ces enjeux patrimoniaux pour la gestion forestière, ce qui est le but de leur inclusion dans les documents d’aménagement. 
...
En concluon, ce projet aboutit à une fonction permettant de récupérer les données ayant trait aux enjeux patrimoniaux depuis le géoportail de l’urbanisme et les différents documents qui y sont hébergés. Cette fonction permet également de trier les informations récupérées pour              s’adapter aux besoins de l’utilisateur, mais aussi de récupérer des informations plus générales sur un massif, essentielles pour rédiger un document d’aménagement forestier. 
